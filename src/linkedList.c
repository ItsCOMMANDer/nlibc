...
linkedListHead_t linkedList_create(uint64_t dataSize, int32_t (*compare)(void*, void*, const linkedListHead_t*)) {
	linkedListHead_t head;

	head.firstNode = NULL;
	head.lastNode = NULL;
	head.nodes = 0;
	head.dataSize = dataSize;
	head.data_compare = compare;
	head.memory_allocate = malloc;
	head.memory_deallocate = free;
...
void linkedList_delete(linkedListHead_t *head) {
	...
	if(head->nodes == 1) {
		head->memory_deallocate(head->firstNode);
		return;
	}
...
void linkedList_append(linkedListHead_t *head, void* data) {
	...
	linkedListNode_t *newNode = head->memory_allocate(sizeof(linkedListNode_t) + head->dataSize);
...
int64_t partition(linkedListHead_t *head, int64_t low, int64_t high) {
	...
	while(i < j) {
		...
		while((head->data_compare(linkedList_get(head, j), pivot, head) == 1) && (j >= low + 1)) {j--;}
...
void linkedListInternal_qsort(linkedListHead_t *head, int64_t low, int64_t high) {
	if(low < high) {
		int64_t partitionIndex = partition(head, low, high);

		linkedListInternal_qsort(head, low, partitionIndex - 1);
		linkedListInternal_qsort(head, partitionIndex + 1, high);
	}
}
...
void linkedList_qsort(linkedListHead_t *head) {
	if(head->nodes == 0) return;
	if(head->nodes == 1) return;
	linkedListInternal_qsort(head, 0, head->nodes - 1);
	return;
}
